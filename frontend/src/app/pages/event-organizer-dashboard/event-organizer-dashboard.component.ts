...
export class EventOrganizerDashboardComponent implements OnInit {
  ...
  eventSearchTerm = ''; eventStatusFilter = 'all'; eventTypeFilter = 'all';
  eventCategoryFilter = 'all'; startDateFilter = ''; endDateFilter = '';
  ...
  selectedImageFile: File | null = null; imagePreviewUrl = '';
  ...
  loadEvents() {
    ...
    this.eventService.getAllEvents(filters).subscribe({
      next: (evts: any[]) => {
        // Apply client-side status computation
        this.events = evts.map(e => ({ ...e, status: this.eventService.computeStatus(e) })) as ApiEvent[];
  ...
  applyLocalFilters() {
    ...
    const status = this.eventStatusFilter;
    const type   = this.eventTypeFilter;
    const cat    = this.eventCategoryFilter;
    ...
    this.filteredEvents = this.events.filter(e => {
      ...
      // Status filter — applied client-side too for reliability
  ...
  onFilterChange() { this.applyLocalFilters(); this.loadEvents(); } // client-side instant + server refresh
  ...
  onImageSelected(event: any) {
    ...
    if (!allowed.includes(file.type)) { this.formError = 'Only JPEG, PNG, GIF or WebP.'; return; }
    if (file.size > 5 * 1024 * 1024) { this.formError = 'Image must be < 5 MB.'; return; }
  ...
  openParticipants(event: ApiEvent) {
    ...
    this.eventService.getEventRegistrations(event._id).subscribe({
      next:  (data: any) => {
        // Normalize: always ensure registrationId is a plain string
        ...
        console.log('[Participants loaded]', this.participants.length, this.participants[0]);
      },
      error: (err: any) => {
  ...
  approveRegistration(regId: string) {
    if (!regId || regId === 'undefined') {
      alert('Registration ID is missing. Please close and reopen the participants panel.'); return;
    }
    this.eventService.updateRegistrationStatus(regId, 'approved').subscribe({
      next: () => {
        const p = this.participants.find(p => String(p.registrationId) === String(regId));
        if (p) p.approvalStatus = 'approved';
        this.notifService.reload();
        this.bulkActionStatus = 'success'; this.bulkActionMessage = 'Registration approved!';
  ...
  openRejectModal(regId: string) {
    if (!regId) { alert('Registration ID is missing. Please close and reopen the participants panel.'); return; }
    this.rejectingRegistrationId = String(regId);
    this.rejectReason = '';
    this.showRejectModal = true;
  }
  closeRejectModal() { this.showRejectModal = false; this.rejectingRegistrationId = ''; }

  confirmReject() {
    if (!this.rejectingRegistrationId || this.rejectingRegistrationId === 'undefined') {
      alert('Registration ID is missing. Please close and reopen the participants panel.'); return;
    }
    this.eventService.updateRegistrationStatus(this.rejectingRegistrationId, 'rejected', this.rejectReason).subscribe({
      next: () => {
        const p = this.participants.find(p => String(p.registrationId) === String(this.rejectingRegistrationId));
        if (p) { p.approvalStatus = 'rejected'; p.rejectionReason = this.rejectReason; }
        this.closeRejectModal();
        this.notifService.reload();
        this.bulkActionStatus = 'success'; this.bulkActionMessage = 'Registration rejected.';
        setTimeout(() => this.bulkActionStatus = null, 3000);
      },
  ...
  toggleParticipantSelection(id: string) {
    const sid = String(id);
    if (this.selectedParticipantIds.has(sid)) this.selectedParticipantIds.delete(sid);
    else this.selectedParticipantIds.add(sid);
  }
  isAllSelected() { return this.participants.length > 0 && this.selectedParticipantIds.size === this.participants.length; }
  toggleSelectAll() {
    if (this.isAllSelected()) this.selectedParticipantIds.clear();
    else this.participants.forEach(p => this.selectedParticipantIds.add(String(p.registrationId)));
  }
  ...
  bulkApprove() {
    ...
    this.eventService.bulkUpdateRegistrationStatus(ids, 'approved').subscribe({
      next: (res: any) => {
        ...
        this.participants.forEach(p => { if (this.selectedParticipantIds.has(String(p.registrationId))) p.approvalStatus = 'approved'; });
        this.selectedParticipantIds.clear();
        this.notifService.reload();
        setTimeout(() => this.bulkActionStatus = null, 3000);
      },
      error: err => { this.bulkActionStatus = 'error'; this.bulkActionMessage = err.error?.message || 'Failed'; }
    });
  }

  // ── BULK REJECT ───────────────────────────────────────────────────────────
  openBulkRejectModal() { if (this.selectedParticipantIds.size === 0) return; this.bulkRejectReason = ''; this.showBulkRejectModal = true; }
  closeBulkRejectModal() { this.showBulkRejectModal = false; }
  ...
  confirmBulkReject() {
    ...
    this.eventService.bulkUpdateRegistrationStatus(ids, 'rejected', this.bulkRejectReason).subscribe({
      next: (res: any) => {
        this.bulkActionStatus  = 'success';
        this.bulkActionMessage = res.message;
        this.participants.forEach(p => { if (this.selectedParticipantIds.has(p.registrationId)) { p.approvalStatus = 'rejected'; p.rejectionReason = this.bulkRejectReason; } });
        this.selectedParticipantIds.clear();
  ...
  closeNotifDropdown()      { this.showNotifDropdown = false; }
  markNotifRead(n: any)     { this.notifService.markRead(n._id); }
  markAllNotifsRead()       { this.notifService.markAllRead(); }
  deleteNotif(n: any)       { this.notifService.delete(n._id); }

  // ── PROFILE MODAL ─────────────────────────────────────────────────────────
  openProfileModal()  { this.showProfileModal  = true; this.showNotifDropdown = false; }
  closeProfileModal() { this.showProfileModal  = false; }

  getInitial():   string { return this.authService.getFullName()?.charAt(0)?.toUpperCase() || 'A'; }
  getFullName():  string { return this.authService.getFullName() || 'Admin'; }
  getEmail():     string { return this.authService.getEmail()    || ''; }
  getRole():      string { return this.authService.getRole()     || 'college-admin'; }
  getCollege():   string { return (this.authService.getUser() as any)?.college || '—'; }
  ...
  getNotifIcon(type: string): string {
    const map: any = { 'new-registration': '📋', 'registration-approved': '✅', 'registration-rejected': '❌', 'event-update': '📢', 'general': '🔔' };
    return map[type] || '🔔';
  }
}
